...
float[] sp = new float[] { randomSpeed(0.5,2), randomSpeed(0.5,2), randomSpeed(0.5,2), randomSpeed(0.5,2) }; 
UDP udp;
int fadeOutUntil = 0;
int fadeInUntil = 30;
...
void drawSpotlights() {
  incrementJEvery(1,1);
  resetFixedGradient();
  mirroredFixedGradient(10,20,jValue,color(0),PURPLE,false);
  mirroredFixedGradient(10,20,wraparound(jValue+25,100),color(0),BLUE,true);
  mirroredFixedGradient(10,20,wraparound(jValue+50,100),color(0),GREEN,true);
  mirroredFixedGradient(10,20,wraparound(jValue+75,100),color(0),PINK,true);
  applyFixedGradient(0,11); 
}

float randomSpeed(float min, float max) {
  float s = random(max-min)+min;
  if (random(2) < 1) { s = -s; }
  return s;
}

void drawRandomSpotlights() {
  resetFixedGradient();
 
  for (int i=0; i<pos.length; i++) {
    pos[i] = pos[i] + sp[i];
    mirroredFixedGradient(10,20,wraparound(int(pos[i]),100),color(0),PALETTE[i],true);
  }
...
void fade(int amount) {
  for (int i=0; i<values.length; i++) {
    values[i] = blendColor(values[i],color(255-amount),MULTIPLY);
  }
}
...
void newMode() {
  ...
  if (frameCount > 1) 
    fadeOutUntil = frameCount + 30;
}

void draw() {
  if (frameCount < fadeOutUntil) {
    fade(20);
  }
  else if (frameCount == fadeOutUntil) {
    fadeInUntil = frameCount + 30;
  }
  else { 
    ...
    if (frameCount < fadeInUntil) {
      int t = fadeInUntil-frameCount;
      int v = int(t/30.0*255);
...
void mirroredFixedGradient(int step, int steps, color c1, color c2) {
   mirroredFixedGradient(step,steps,0,c1,c2,false);
}

void mirroredFixedGradient(int step, int steps, int offset, color c1, color c2, boolean blending) {
  // calculate differences between color components 
  float deltaR = red(c2)-red(c1);
  float deltaG = green(c2)-green(c1);
  float deltaB = blue(c2)-blue(c1);
  ...
  for (int i=0; i<s2; i++) {
    j=i+step+offset;
    if (j>=fixedGradient.length) { j=j-fixedGradient.length; }
    ...
    if (blending) {
      c = blendColor(fixedGradient[j],c,ADD);
    }
